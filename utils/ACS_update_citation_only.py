...
# sql_conn = pyodbc.connect('DRIVER={ODBC Driver 17 for SQL Server}; SERVER=George-PC; DATABASE=ACS;   UID=datarw;   PWD=Toronto2020')
    with sql_conn.cursor() as cursor:
        ...
        #
        #
        query='SELECT article_info_id, abstract_id, subject_index_id, pub_yr, pub_dt, doi_url ' \
              'FROM article_info where assigned=0 and retrieved=0 order by RAND() limit 10;'
        #
        abstract_list=pd.read_sql(query, sql_conn_df)
        print(abstract_list)
        #
        update_sql = "Update article_info set assigned=1 where article_info_id in (" + ', '.join(list(map(str, abstract_list.iloc[:,0].tolist()))) + ")"
        #
        ...
        for i in range(0,len(abstract_list)):
            ...
            article_info_id=abstract_list.iloc[i,0]
            abstract_id=abstract_list.iloc[i,1]
            subject_index_id = abstract_list.iloc[i, 2]
            pub_yr = abstract_list.iloc[i, 3]
            pub_dt = abstract_list.iloc[i, 4]
            doi_url = abstract_list.iloc[i, 5]
            ...
            URL=abstract_list.iloc[i,5].replace(" ","")
            ...
            if re.match('http', URL):
                ...
                #
                # print("article_info_id= ", article_info_id)
                # print("abstract_id= ", abstract_id)
                # print("subject_index_id= ", subject_index_id)
                # print("pub_yr= ", pub_yr)
                # print("pub_dt= ", pub_dt)
                ...
                # print("doi_url= ", doi_url)
                # print("views= ", views)
                # print("altmetric= ", altmetric)
                # print("citations= ", citations)
                # print("subject= ", subject)
                # # print("abstract= ", abstract)
                # print("cited_by= ", cited_by)
                # print("cited_by_doi_url= ", cited_by_doi_url)
                # print("cited_by_json= ", cited_by_json)
                # print("cited_by_doi_url_json= ", cited_by_doi_url_json)

                qry_update = f"update article_info set views='{views}', altmetric='{altmetric}', citation='{citations}', " \
                             f"update_dt=now(), retrieved=1 where article_info_id={article_info_id}"
                qry_insert = f"""insert into article_info_citation_history (
                            article_info_id, abstract_id, subject_index_id, doi_url,
                            pub_yr, pub_dt, views, altmetric, citation,
                            cited_by_doi_url, cited_by)
                            VALUES
                            (
                            {article_info_id}, {abstract_id}, {subject_index_id}, '{doi_url}',
                            {pub_yr}, cast('{pub_dt.strftime('%Y-%m-%d')}' as date), '{views}', '{altmetric}', '{citations}',
                            '{cited_by_doi_url}', '{cited_by}')"""
                # print(type(pub_dt))
